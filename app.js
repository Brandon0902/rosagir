(() => {
  ...
  function CreateMagicDust(x1, x2, y1, y2, sizeRatio, fallingTime, animationDelay, node = 'main') {
    ...
    if (container) container.appendChild(dust);
  }
  ...
  // =========================
  //  🎁 TARJETA: abrir/cerrar
  // =========================
  const cardBtn = document.getElementById('giftCard');
  if (cardBtn) {
    cardBtn.addEventListener('click', () => {
      const isOpen = cardBtn.classList.toggle('is-open');
      cardBtn.setAttribute('aria-expanded', String(isOpen));
    });
  }
})();
